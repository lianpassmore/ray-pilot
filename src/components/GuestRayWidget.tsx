...
interface GuestRayWidgetProps {
  userName: string;
}

export default function GuestRayWidget({ userName }: GuestRayWidgetProps) {
  ...
  const startConversation = useCallback(async (startMode: 'voice' | 'text' = 'voice') => {
    try {
      ...
      const response = await fetch(`/api/elevenlabs-guest?name=${encodeURIComponent(userName)}`);
      ...
      await conversation.startSession({
        ...
        dynamicVariables: {
          ...
          session_number: '1',
          session_type: 'guest',
          user_context: 'No personal context provided.',
          days_since_last_session: 'first session',
